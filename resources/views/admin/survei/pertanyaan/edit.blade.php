...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <form action="{{ route('admin.survei.pertanyaan.update', [$survei->id, $pertanyaan->id]) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="pertanyaan" class="block text-sm font-medium text-gray-700 mb-1">Pertanyaan <span class="text-red-600">*</span></label>
                <input type="text" name="pertanyaan" id="pertanyaan" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200" value="{{ old('pertanyaan', $pertanyaan->pertanyaan) }}" required>
                @error('pertanyaan')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="mb-4">
                    ...
                    <select name="tipe" id="tipe" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200" required>
                        ...
                        <option value="text" {{ old('tipe', $pertanyaan->tipe) == 'text' ? 'selected' : '' }}>Text (Isian)</option>
                        <option value="radio" {{ old('tipe', $pertanyaan->tipe) == 'radio' ? 'selected' : '' }}>Radio (Pilihan Tunggal)</option>
                        <option value="checkbox" {{ old('tipe', $pertanyaan->tipe) == 'checkbox' ? 'selected' : '' }}>Checkbox (Pilihan Ganda)</option>
                        <option value="select" {{ old('tipe', $pertanyaan->tipe) == 'select' ? 'selected' : '' }}>Select (Dropdown)</option>
                    </select>
                    @error('tipe')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="urutan" id="urutan" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200" value="{{ old('urutan', $pertanyaan->urutan) }}" min="1" required>
                    @error('urutan')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <div class="flex items-center">
                        <label class="inline-flex items-center">
                            <input type="checkbox" name="wajib" class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" {{ old('wajib', $pertanyaan->wajib) ? 'checked' : '' }}>
